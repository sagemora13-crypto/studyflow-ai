...
enum SampleData {
  static func seed(into context: ModelContext, subjects: [String]) {
    ...
    algebra.cards = [
      Flashcard(
        front: "What is a variable?", back: "A symbol (like x) that stands for an unknown number."),
    ...
    bio.cards = [
      ...
      Flashcard(
        front: "What does the nucleus do?", back: "Stores DNA and controls the cell's activities."),
      Flashcard(
        front: "Photosynthesis converts...", back: "Light energy into chemical energy (glucose)."),
    ]
    ...
    // Assignments
    let a1 = Assignment(
      title: "Chapter 4 problem set", subject: "Math", dueDate: daysFromNow(1), difficulty: 2,
      estimatedMinutes: 40, priority: 3)
    a1.steps = [
      AssignmentStep(detail: "Review examples 4.1–4.3", dayOffset: 0),
      AssignmentStep(detail: "Do odd problems 1–15", dayOffset: 0),
    ]
    let a2 = Assignment(
      title: "Lab report: osmosis", subject: "Science", dueDate: daysFromNow(3), difficulty: 3,
    ...
    context.insert(
      StudySession(subject: "Math", activity: "Flashcards", minutes: 20, focusRating: 3))
    context.insert(
      QuizResult(
        topic: "Fractions", subject: "Math", scorePercent: 80, totalQuestions: 5, correctCount: 4,
        weakTopics: ["Mixed numbers"]))
  }
