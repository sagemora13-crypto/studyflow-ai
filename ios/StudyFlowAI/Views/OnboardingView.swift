...
struct OnboardingView: View {
    ...
    private var nameStep: some View {
        stepContainer(
            ...
            TextField("First name or nickname", text: $name)
                ...
                .overlay(RoundedRectangle(cornerRadius: Theme.radiusMedium, style: .continuous).strokeBorder(Theme.hairline))
        }
    }
        ...
        stepContainer(title: "Which subjects?", subtitle: "Pick the ones you want help with first.") {
            VStack(spacing: 10) {
                ForEach(Subject.allCases) { subject in
                    selectRow(title: subject.rawValue, icon: subject.icon, selected: subjects.contains(subject.rawValue)) {
                        if subjects.contains(subject.rawValue) { subjects.remove(subject.rawValue) }
                        else { subjects.insert(subject.rawValue) }
                    }
                }
            }
        }
    ...
    private var styleStep: some View {
        stepContainer(title: "How do you learn best?", subtitle: "This shapes how the AI explains things.") {
            VStack(spacing: 10) {
    ...
    private var goalStep: some View {
        stepContainer(title: "Set your daily goal", subtitle: "Small, consistent sessions build streaks.") {
            VStack(alignment: .leading, spacing: 18) {
                VStack(alignment: .leading, spacing: 8) {
                    Text("\(minutes) min / day")
                        .font(.title2.weight(.bold))
                        .foregroundStyle(Theme.accent)
                    Slider(value: Binding(get: { Double(minutes) }, set: { minutes = Int($0) }), in: 10...90, step: 5)
                        .tint(Theme.accent)
                }
    ...
    private func selectRow(title: String, icon: String? = nil, selected: Bool, action: @escaping () -> Void) -> some View {
...
#Preview {
    OnboardingView()
        .modelContainer(for: [StudentProfile.self, Assignment.self, AssignmentStep.self, TestDate.self, Deck.self, Flashcard.self, QuizResult.self, StudySession.self, StudyGuide.self, EarnedBadge.self], inMemory: true)
