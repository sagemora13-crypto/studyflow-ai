...
struct PlannerView: View {
                    ...
                    VStack(alignment: .leading, spacing: 16) {
                        if assignments.isEmpty {
                            EmptyStateView(
                                ...
                                message: "Add an assignment and StudyFlow keeps it front and center until it's done.",
    ...
    private func assignmentCard(_ item: Assignment) -> some View {
        ...
        .card()
        .swipeActions {
            Button(role: .destructive) { context.delete(item) } label: { Label("Delete", systemImage: "trash") }
        }
    }
...
struct NewAssignmentSheet: View {
            ...
            Form {
                ...
                Section("Effort") {
                    Picker("Difficulty", selection: $difficulty) {
                        Text("Easy").tag(1); Text("Medium").tag(2); Text("Hard").tag(3)
                    }
    ...
    private func save() {
        context.insert(Assignment(title: title, subject: subject, dueDate: dueDate, difficulty: difficulty, estimatedMinutes: minutes, priority: difficulty))
        dismiss()
    }
}
