      ...
      if decks.isEmpty {
        ...
        ForEach(decks) { deck in
          HStack {
            ...
            Spacer()
            ProgressRing(
              progress: deck.mastery, size: 40, lineWidth: 5,
              color: Theme.subjectColor(deck.subject))
          }
          .card(padding: 14)
        }
      }
  ...
  private var preferences: some View {
    VStack(alignment: .leading, spacing: 12) {
      ...
      VStack(alignment: .leading, spacing: 8) {
        ...
        Slider(
          value: Binding(
            get: { Double(profile.dailyStudyMinutes) }, set: { profile.dailyStudyMinutes = Int($0) }
          ), in: 10...90, step: 5
        )
        .tint(Theme.accent)
      }
